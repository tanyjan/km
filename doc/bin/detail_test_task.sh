...
nohup $JAVA $JAVA_OPTS $TIMEZONE "-DLOG_FILE_DIR=logs/WorkenDetailXpathServer_test" -cp "$CONF_DIR:$APP_DIR/lib/webant-xpath-0.0.1.jar:$LIB_JARS" cn.inveno.worken.WorkeDetailTest $CFG_NAME > /dev/null 2>&1  &
#$JAVA $JAVA_OPTS -cp "$CONF_DIR:$APP_DIR/lib/webant-xpath-0.0.1.jar:$LIB_JARS" cn.inveno.worken.WorkeDetailTest $CFG_NAMEi
